...
```text
https://github.com/Mio0707/Owen-animal-band.git
```
...
```text
安装 Skill → 上传简谱与原曲 → Qwen 识谱草稿 → 打开“简谱检查”
...
需要 Python 3.9+ 与 Node.js。在线识谱还需要环境变量 `DASHSCOPE_API_KEY`，也可写入根目录 `.env`；该文件不会提交到 Git。
...
发行架构是 `Skill + Local CLI + Animal Band Engine + Static Classroom + Web Sampler`。专业 Engine 是课程判断与备课生成的唯一事实来源；课堂导出后不依赖服务器、Qwen、CLI、FluidSynth 或 MCP。
...
公开或商业分发前请阅读 [LICENSES.md](LICENSES.md)。Web Sampler 当前仍标记为 `LICENSE_REVIEW_REQUIRED`。
