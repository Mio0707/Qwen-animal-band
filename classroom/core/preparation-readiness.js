...
export function evaluatePreparationReadiness(input) {
  const {
    ...
    measureAlignment, listeningBodyPlan, stickerStemPack, webSampler,
  } = input;
  ...
  if (requirements.has("STICKER_STEMS")) {
    ...
    const instruments = webSampler?.instruments ?? {};
    const ready = stickerStemPack?.songId === preparation?.songId
      && stickerStemPack?.sourceScoreVerifiedAt === verifiedScore?.verifiedAt
      && fixed.every((trackId) => Array.isArray(byId.get(trackId)?.events))
      && fixed.every((trackId) => Array.isArray(instruments?.[trackId]?.samples) && instruments[trackId].samples.length > 0)
      && stickerStemPack?.qa?.aligned !== false;
    checks.push(check(
      "WEB_SAMPLER_READY",
      ready,
      "Web Sampler 与四轨 Event JSON 已准备",
      "请检查 Web Sampler 和 dog/bear/cat/lion 四轨事件"
    ));
  }
  ...
  return { ready, desiredPreparationStatus: ready ? "READY" : "DRAFT", readinessMode: "QWENWORK_LOCAL_CLI_WEB_AUDIO_V2", checks, blockers: unique, warnings };
}
