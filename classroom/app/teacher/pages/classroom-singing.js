...
export function renderSingingActivity(data, params) {
  ...
  const hasOriginalAudio=Boolean(song?.assets?.originalAudio);
  const originalReady=Boolean(hasOriginalAudio&&(alignment?.calibration?.startSec!=null||alignment?.anchors?.some((item)=>Number(item.measure)===Number(units[0]?.startMeasure??1))));
  const intro=hasOriginalAudio?"可以用钢琴音高、唱名或老师上传的原曲来学习。":"当前为简谱模式：使用钢琴音高和唱名练唱；补充原曲后可解锁原曲跟唱。";
  return `<main class="student-activity singing-activity" data-singing-runtime data-song-id="${escapeHtml(preparation.songId)}" data-bpm="${escapeHtml(recipe.songContext?.bpm??72)}" data-song-audio-url="${escapeHtml(assetUrl(song?.assets?.originalAudio))}">
    <header class="student-activity-header"><a class="back-link" href="#/song?id=${encodeURIComponent(preparation.songId)}&step=lesson">← 返回课堂方案</a><p class="eyebrow">${mode==="live"?"正式课堂":"备课预览"} · 学演唱</p><h1>一段一段唱</h1><p>每 ${escapeHtml(measuresPerUnit)} 小节一段。${intro}</p></header>
